...
Delivery requires server-owned `RESEND_API_KEY`, `AUTH_EMAIL_FROM` for a verified sender, and `TOKEN_ENCRYPTION_KEY`. As checked on 2026-09-20, production has encryption configured but still needs the provider key and sender; do not claim live email delivery until configuration and delivery are verified.
