 ...
 * Existing required fields and their meaning are frozen in v1. Add OPTIONAL
 * fields, preserve nullability, and ignore unknown fields. Breaking changes
 * require a parallel v2 contract and an explicit migration, never a rename.
...
export type SourceName =
  "gobblerconnect" | "vt-sports" | "vt-events" | "canvas" | "discord" | "demo";
...
export interface DiscoveryRequest {
  ...
  /** Demo input only. Live identity/preferences ALWAYS come from server session. */
  profile?: Profile;
  saved?: Id[];
  feedback?: Record<Id, number>;
}
...
export interface HttpApi {
  ...
  /** GET /api/bootstrap. Public read of server-owned defaults; no writes. */
  bootstrap: Operation<void, BootstrapView>;
  /** GET /api/events?mode=. Public listings; no writes. */
  listEvents: Operation<{ mode: Mode }, EventList>;
  /** GET /api/events/:id/ics?mode=. Returns calendar text; does NOT write any calendar. */
  exportCalendar: Operation<{ eventId: Id; mode: Mode }, string>;
  ...
  /** POST /api/discovery. Read-only search/ranking; demo never reads private context. */
  ...
  /** POST /api/demo/assistant. Deterministic sample matching; no external AI or persistence. */
  askDemo: Operation<{ query: string; profile?: Profile }, AssistantReply>;
