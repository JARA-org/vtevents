...
test("Vultr release preserves settings and rolls back failed replacement or smoke checks", { skip: !available }, () => {
  ...
  try {
    for (const scenario of ["success", "preflight", "health", "smoke", "rollback"]) {
...
`;
      ...
      assert.equal(calls.includes("/opt/old/deploy/compose.yaml up"), ["health", "smoke", "rollback"].includes(scenario));
